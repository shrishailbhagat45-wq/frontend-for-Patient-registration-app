...
export default function ListPrescription({prescription, idx}) {
  ...
  const remainingCount = drugCount - 3;  return (
    <div
      key={idx}
      className="group bg-white rounded-lg shadow-sm border border-slate-200 hover:shadow-md hover:border-blue-200 transition-all duration-200 overflow-hidden w-full max-w-sm h-[480px] flex flex-col"
    >
      ...
      <div className="p-4 flex-1 overflow-y-auto">
        <div className="mb-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="px-2 py-0.5 bg-blue-50 text-blue-700 rounded-full text-xs font-semibold flex-shrink-0">
              {drugCount}
            </span>
          </div>
          ...
          <div className="space-y-2">
            {displayDrugs.map((drug, idx) => (
              <div 
                key={idx} 
                className="flex items-center justify-between p-2.5 bg-slate-50 rounded-lg hover:bg-blue-50 transition-all duration-200 border border-slate-200 hover:border-blue-200"
              >
                <div className="flex-1 min-w-0 pr-2">
                  <p className="font-semibold text-slate-900 text-xs truncate">
                    {drug.name}
                    {drug.strength && <span className="text-slate-600 ml-1">{drug.strength}</span>}
                  </p>
                  {drug.remarks && (
                    <p className="text-xs text-slate-500 mt-0.5 truncate">{drug.remarks}</p>
                  )}
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  <div className="text-center px-1.5">
                    <p className="text-xs text-slate-500 font-medium">Qty</p>
                    <p className="text-xs font-bold text-slate-800">{drug.quantity}</p>
                  </div>
                  <div className="h-6 w-px bg-slate-300"></div>
                  <div className="text-center px-1.5">
                    <p className="text-xs text-slate-500 font-medium">Freq</p>
            ...
            {remainingCount > 0 && (
              <div className="text-center py-1.5 px-2.5 bg-blue-50 rounded-lg border border-blue-100">
                <p className="text-xs font-medium text-blue-600">
                  +{remainingCount} more medication{remainingCount > 1 ? 's' : ''}
                </p>
              </div>
            )}
          </div>
          ...
          <div className="mt-3 pt-3 border-t border-slate-200">
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-700 line-clamp-2">{prescription.remarks}</p>
              </div>
            </div>
          </div>
        )}
